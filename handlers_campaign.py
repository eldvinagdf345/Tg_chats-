...
async def _show_confirm(call: CallbackQuery, account_id: int, count: int):
    ...
    await call.message.edit_text(
        ...
        f"Стиль и цель общения берутся из инструкций аккаунта. Новые диалоги стартуют "
        f"в режиме черновиков — ответы будут приходить вам на проверку. Уже начатые "
        f"диалоги пропускаются.",
